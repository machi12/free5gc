...
import (
	...
	"github.com/machi12/openapi/Nudr_DataRepository"
	"github.com/machi12/openapi/models"
	"github.com/machi12/util/httpwrapper"
	"github.com/machi12/util/milenage"
	"github.com/machi12/util/ueauth"
)
...
func GenerateAuthDataProcedure(authInfoRequest models.AuthenticationInfoRequest, supiOrSuci string) (
	...
	*/
	...
	// NOTE: Delete operations related to SQN
	...
	// NOTE: Modify the log
	logger.UeauLog.Tracef("K=[%x], OP=[%x], OPC=[%x]", k, op, opc)
	...
	// NOTE: Delete operations related to AMF
	...
	// NOTE: Modify the log
	logger.UeauLog.Tracef("RAND=[%x]", RAND)

	// NOTE: Delete operations related to re-synchronization
	//// re-synchronization
	...
	// NOTE: Delete operations related to SQN incrementation
	// increment sqn
	//bigSQN := big.NewInt(0)
	//sqn, err = hex.DecodeString(sqnStr)
	//if err != nil {
	...
	// Run milenage
	// NOTE: Rename macA to HNMAC and remove macS
	HNMAC := make([]byte, 8)
	...
	// NOTE: Generate HNMAC
	err = milenage.F1(opc, k, RAND, nil, nil, HNMAC, nil)
	if err != nil {
		logger.UeauLog.Errorln("milenage F1 err:", err)
	}

	// NOTE: Generate RES, CK, IK, AK
	// RES == XRES (expected RES) for server
	err = milenage.F2345(opc, k, RAND, RES, CK, IK, AK, nil)
	if err != nil {
	...
	// NOTE: Modify the computation method for AUTN
	for i := 0; i < len(HNMAC); i++ {
		AUTN[i] = HNMAC[i] ^ AK[i]
	}
	...
	if authSubs.AuthenticationMethod == models.AuthMethod__5_G_AKA {
		...
		P0 = []byte(authInfoRequest.ServingNetworkName)
		// NOTE: Use the first 6 bytes of AUTN as P1
		P1 = AUTN[0:6]
		...
		// NOTE: Need to send an additional HNMAC
		av.HNMAC = hex.EncodeToString(HNMAC)
		...
		P0 := []byte(authInfoRequest.ServingNetworkName)
		// NOTE: Since the SQN-related process was commented out earlier, to avoid errors, here use the first 6 bytes of AUTN as P1 (due to the structure of Free5GC's code necessitating this modification)
		P1 := AUTN[0:6]
